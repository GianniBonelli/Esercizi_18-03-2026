 ...
 * This is a software in C language for computing numerical integration * by adopting different techniques.
   ...
   Available techniques: Riemann sum, Trapezoidal Rule.
...
int main() {
	...
	float fcoeff[6] = {-10.0,1.0,0.0,2.0,4.0,1.0};	/* the polynomial */
	float xmin = 0.0; 					/* the integration range */ 
	float xmax= 5.0; 					
	int intervals = 10000; 					/* number of equally spaced intervals */ 
	...
	for (i=0; i<=intervals; i++) {
		fvalues[i] = Polynomial(fcoeff,6,in);
		in += gap;
	}
